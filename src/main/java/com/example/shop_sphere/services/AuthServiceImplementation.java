...
import com.example.shop_sphere.entities.JWTToken;
...
public class AuthServiceImplementation implements AuthService {
	...
	public AuthServiceImplementation(UserRepository userRepository, JWTTokenRepository jwtTokenRepository,
	...
	public void logout(User user) {
		int userld = user.getUserId();
		// Retrieve the JWT token associated with the user
		JWTToken token = jwtTokenRepository.findByUserId(userld);
		// If a token exists, delete it from the repository
		if (token != null) {
			jwtTokenRepository.deleteByUserId(userld);
		}
	}
