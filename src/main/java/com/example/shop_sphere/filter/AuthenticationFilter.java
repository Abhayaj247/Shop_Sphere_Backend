package com.example.shop_sphere.filter;
...
public class AuthenticationFilter implements Filter {
    ...
    private static final String[] UNAUTHENTICATED_PATHS = {
        "/api/users/register",
        "/api/auth/login",
        "/api/auth/logout"   // ⭐ ADDED — allow logout without token & role check
    };
    ...
    private void executeFilterLogic(ServletRequest request, ServletResponse response, FilterChain chain)
        ...
        // ⭐ ADDED — set CORS headers for ALL requests
        setCORSHeaders(httpResponse);

        // ⭐ UPDATED — allow unauthenticated paths including logout
        ...
        if (httpRequest.getMethod().equalsIgnoreCase("OPTIONS")) {
            return; // ⭐ UPDATED — headers already set above
        }
        ...
        // ⭐ ADDED — Skip role check for logout
        ...
        // ⭐ UPDATED — CORS header already set above
        chain.doFilter(request, response);
    }
